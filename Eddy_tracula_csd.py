...
templates = {'dwi':   '/om/project/FACT/subjects/{subject_id}/dti/dti_001.nii.gz',
             'bvals': '/om/project/FACT/subjects/{subject_id}/dti/dti_001.bvals',
             'bvecs': '/om/project/FACT/subjects/{subject_id}/dti/dti_001.bvecs'}

#PE_order = ['AP', 'PA']
#select = MapNode(SelectFiles(templates), iterfield=['PE'], name='select')
select = Node(SelectFiles(templates), name='select')
...
def run_prep(sid, template, nifti, bvec, bval):
    ...
    node = Node(CommandLine('trac-all -prep -c %s -no-isrunning' % config_file, terminal_output='allatonce'),
...
eddy.plugin_args = {'sbatch_args': '--qos=gablab --mem=10G -c 4'}
...
wf.connect(select, 'dwi', node1, 'nifti')
wf.connect(select, 'bvals', node1, 'bval')
wf.connect(select, 'bvecs', node1, 'bvec')

node2 = Node(Function(input_names=['sid', 'tracula_dir'],
                     output_names=['sid'], function=run_bedpost),
            name='trac-bedp')
node2.inputs.tracula_dir = tracula_dir
node2.plugin_args = {'sbatch_args': '--gres=gpu:1 --time=12:00:00',
                      'overwrite': True}
...
tracker.plugin_args = {'sbatch_args': '--mem=%dG -N 1 -c %d --time=12:00:00' % (10 * num_threads, #3 * num_th
                                                                                    num_threads),
                       'overwrite': True}
...
wf.run('SLURM', plugin_args={'sbatch_args': '-N1 -c2 --mem=100G --time=24:00:00'})
